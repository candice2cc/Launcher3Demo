...
public class Launcher extends Activity
    ...
    protected boolean hasCustomContentToLeft() {
        ...
        // edit by candice:使用配置参数判断是否存在左屏
...
//                ? R.id.workspace_blocked_row : R.id.qsb_container);
        ...
        // edit by candice:自定义qsb逻辑，不区分横竖屏
...
//        }
    ...
    public void onClick(View v) {
        ...
        } else if (tag instanceof AdInfo) {
            // edit by candice:广告位点击逻辑入口
        ...
        if (!mDragController.isDragging()) {
            if (itemUnderLongClick == null) {
                ...
                if (mWorkspace.isInOverviewMode()) {
                    ...
                    // edit by candice：注释 长按空白处，进入OverviewMode 功能
//                    showOverviewMode(true);
                }
            } else {
...
//                        }
                        ...
                        // edit by candice：注释原有的 长按进入DeepShortcuts功能，改为长按进入 快捷操作 功能
                    ...
                    // edit by candice：hotseat固定，不允许长按拖拽
    ...
    @Thunk
    ArrayList<AdInfo> mHotSeatAppsList;
    private Runnable mBindHotSeatRunnable = new Runnable() {
        @Override
        public void run() {
            bindHotSeat(mHotSeatAppsList);
            mHotSeatAppsList = null;

        }
    };

    /**
     * edit by candice:增加绑定托盘入口代码
     * @param apps
     */
    public void bindHotSeat(final ArrayList<AdInfo> apps) {
        if (waitUntilResume(mBindHotSeatRunnable, true)) {
            mHotSeatAppsList = apps;
            return;
        }

        if (mHotseat != null) {
            mHotseat.setApps(apps);
        }


        if (mLauncherCallbacks != null) {
            mLauncherCallbacks.bindHotSeat(apps);
        }
    }
